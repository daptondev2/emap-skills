...
description: >-
  ...
  website. Covers Integration 2 (redirect handoff — browser redirects to EMAP
  with step-1 data in URL params, EMAP prefills and auto-submits the form) and
  Integration 3 (API submission — partner backend POSTs step-1 data to EMAP's
  REST API, EMAP creates the record and emails the merchant a continuation link).
...
EMAP handles all subsequent steps (company details, banking, e-signature).
...
1. **Does the project have a server-side backend?**
   - Yes (Node/Express, PHP, Python, Next.js, etc.) → Integration 2 or 3 are both available.
   - No (static site, client-side only) → **Only Integration 2** is available. Integration 3
     requires a backend to hold the partner key server-side. If the developer wants Integration 3
     on a static site, offer to scaffold a serverless function (Vercel, Netlify, Cloudflare Workers).

2. **What framework or language is in use?**
   - Use the matching template from `templates/integration-2/` or `templates/integration-3/`.
...
| | Integration 2 — Redirect handoff | Integration 3 — API submission |
|---|---|---|
| **How it works** | Browser redirects to EMAP `/signup` with step-1 fields in URL params | Partner backend POSTs step-1 fields to EMAP REST API |
| **Where merchant continues** | EMAP, from step 2 onward (immediately after redirect) | EMAP, from step 2 onward (after clicking email link) |
| **Backend required?** | No — redirect is client-side | Yes — `partner_key` must stay server-side |
| **Auto-submit on EMAP?** | Yes, when all non-excluded step-1 fields are provided | N/A — EMAP processes the record server-to-server |
| **Best for** | Simple embed, static sites, fastest integration | More control over UX, need a clean API response |
