...
| Industry Type | `industry_type` | `industry_type` | Optional | **Int 2:** full industry name, e.g. `E-Commerce` — use the `name` field from `/api/partner/industry-types`. **Int 3:** slug, e.g. `e-commerce` — use the `slug` field | Value format differs between modes |
