...
| `federal_tax_id` | Required unless Step 1 country=`CA` — simply that, nothing else. EMAP's signup form also exempts business_organized=`Sole-Proprietorship`, but live testing shows the API does not honor that exemption: a US and a Germany Sole-Proprietorship submitted without this field both got a 422 "required for all companies except Canada and Sole-Proprietorships" — the message names an exemption the API doesn't actually grant. Follow the API's real rule (CA-only exemption), not the signup form's. | Numeric only, masked `XXX-XX-XXXX` (3-2-4) for US/CA/PR, `XX-XXXXXXX` (2-7) otherwise — never free-form alphanumeric, in any country. Use `country_from_step1` (the Step 1 formation country), not `address_country`. |
| `business_register_number` | Required for every Step 1 country except `US` — simply "non-US". EMAP's signup form also exempts PR and CA sole-props, but live testing shows the API requires it for both: a Puerto Rico submission without this field got a live 422. Follow the API's real rule (US-only exemption), not the signup form's. | Max 20 (11 for CA). Use `country_from_step1` (the Step 1 formation country), not `address_country`. |
