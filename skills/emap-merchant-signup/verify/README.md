...
| `conditional_logic` | every `dependsOn`/`visibleIf` field's controlling field id and comparison value are referenced together in the form's JS — inline `<script>` blocks, component code, or JS held in string constants (not just nearby in HTML markup). Also fails a loop over every form control (`form.elements`, `querySelectorAll('input…')` or `[name]`) that copies **disabled** controls into the payload by name: each `usStateSelect` field's hidden text box / US-state dropdown twin is disabled and shares the name, so it overwrites the chosen state (EMAP: "State is required"). Locked fields must be read back by id. |
...
| `error_handling` | Integrations 1 and 3, which `POST` to EMAP, compare the response status to `429` and `422` explicitly, so a rate limit or a validation error isn't shown as a generic failure. Integration 2 is skipped: its only `POST` is the background auto-save, whose responses are deliberately ignored. A textual check: it confirms the branches exist, not what they show |
